 ...
 *
 * This READER process files with extension ".csp".
 *
 * @section READERINCLUDE The SCIPincludeReadercsp() interface method
 *
 * The interface method <code>SCIPincludeReadercsp()</code> is called to add the reader plugin to \SCIP (see
 ...
 * file reader is in charge. In our case we selected the file extension "csp". This means that all files which have
 ...
 * we restrict ourself to the callback <code>READERREAD</code> which is the only one we implemented for the csp
 ...
 * to the \ref probdata_csp.h "main problem data plugin". For this, we use the interface methods
 * SCIPprobdataCreate() which is provided by the
 * problem data plugin (see probdata_csp.c). After that, the reader sets the result value for the SCIP_RESULT
...
#define READER_NAME             "cspreader"
#define READER_DESC             "file reader for csp data format"
#define READER_EXTENSION        "csp"

/**@} */

int getNextLineCSP(SCIP_FILE* file, int* lineno, char* buffer);
...
static
SCIP_DECL_READERREAD(readerReadcsp)
{  /*lint --e{715}*/
    SCIP_FILE* file;
    ...
    *result = SCIP_DIDNOTRUN;
    ...
    {
        /* get next line */
        if(!getNextLineCSP(file, &lineno, buffer))
...
/** includes the csp file reader in SCIP */
SCIP_RETCODE SCIPincludeReadercsp(
   ...
   )
{
    ...
    /* create csp reader data */
    readerdata = NULL;
    
    SCIP_CALL(SCIPincludeReaderBasic(scip, &reader, READER_NAME, READER_DESC, READER_EXTENSION, readerdata));
    assert(reader != NULL);
    
    SCIP_CALL(SCIPsetReaderRead(scip, reader, readerReadcsp));
