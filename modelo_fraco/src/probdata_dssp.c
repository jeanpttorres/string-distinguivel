    ...
    )
{
   if(param.model_novo){
    ...
    /* Number of constraints */
     ...
     SCIP_CALL(SCIPallocBufferArray(scip, &vars, N_VARS_X + N_VARS_D));
	 ...
	 ∀s∈Sc,1≤j≤m
     */
     for (i = 0; i < N_CONS_1; i++) {
	 SCIP_CALL(SCIPcreateConsBasicLinear (scip, &conss[i], "(1)", 0, NULL, NULL, 1.0, 1.0));
	 SCIP_CALL(SCIPaddCons(scip, conss[i]));
     }

     ncons = i;
     for (; i < ncons + N_CONS_2; i++) {
	 SCIP_CALL(SCIPcreateConsBasicLinear (scip, &conss[i], "(2)", 0, NULL, NULL, 0.0, 0.0)); 
	 SCIP_CALL(SCIPaddCons(scip, conss[i]));
     }

     ncons = i;
     for (; i < ncons + N_CONS_3; i++) {
	 SCIP_CALL(SCIPcreateConsBasicLinear (scip, &conss[i], "(3)", 0, NULL, NULL, 0.0, 0.0)); 
	 SCIP_CALL(SCIPaddCons(scip, conss[i]));
     }

     /* (4) constraint - is sci's symbol matched? 

	 ∀s∈Sc, 1≤j≤m, j ≤ i ≤ |s|−m+j
     */
     ncons = i;
     for (; i < ncons + N_CONS_4; i++) {
	 SCIP_CALL(SCIPcreateConsBasicLinear (scip, &conss[i], "(4)", 0, NULL, NULL, -SCIPinfinity(scip), 0.0));
	 SCIP_CALL(SCIPaddCons(scip, conss[i]));
     }

     /* (5) constraint - is sfi's symbol matched? */
     ncons = i;
     for (; i < ncons + N_CONS_5; i++) {
     ...
     */
	 ...
	 for (j = 0; j < string_size; j ++) {
		 ...
		 for (l = 0; l < alpha_size; l++) {
		     (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "x_%d_%d_%d_%c", i, j, k + j, symbol_value(alphabet, alpha_size, l));
		     ...
		     SCIP_CALL(SCIPaddCoefLinear(scip, conss[N_ROWS_1(i, j)], var, 1.0));


		     /* add variable to corresponding constraint */
		     /* add variable to constraint (2) */
		     if (j < string_size - 1 && k < sum_sc[i+1] - sum_sc[i] - 1)
			 SCIP_CALL(SCIPaddCoefLinear(scip, conss[N_ROWS_2(i, j, k)], var, 1.0));

		     if (j > 0 && k < sum_sc[i+1] - sum_sc[i] - 1)
			 SCIP_CALL(SCIPaddCoefLinear(scip, conss[N_ROWS_2(i, j-1, k)], var, -1.0));

		     /* add variable to corresponding constraint */
		     /* add variable to constraint (3) */
		     if  (i == 0)
			 for (int z = 1; z < sc_size; z++)
			     SCIP_CALL(SCIPaddCoefLinear(scip, conss[N_ROWS_3(z, j, l)], var, 1.0));
		     else
			 SCIP_CALL(SCIPaddCoefLinear(scip, conss[N_ROWS_3(i, j, l)], var, -1.0));

		     /* add variable to corresponding constraint */
		     /* add variable to constraint (4) */
		     if (symbol_value(alphabet, alpha_size, l) != sc[i][k + j])
			 SCIP_CALL(SCIPaddCoefLinear(scip, conss[N_ROWS_4(i)], var, 1.0));

		     /* add variable to corresponding constraint */
		     /* add variable to constraint (5) */
		     if (i == 0)
			 for (int z = 0; z < sf_size; z++)
			     for (int w = 0; w < sum_sf[z+1] - sum_sf[z]; w++)
    ...
    assert(var != NULL);
     ...
     /* add variable to corresponding constraint */
     /* add variable to constraint (5) */
     for (i = 0; i < N_CONS_5; i++) {
	 SCIP_CALL(SCIPaddCoefLinear(scip, conss[N_CONS_4 + N_CONS_3 + N_CONS_2 + N_CONS_1 + i], var, 1.0));
     }
